                    ...
                    addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if(task.isSuccessful()) {
                                database.getReference().child("User").orderByKey().equalTo(auth.getCurrentUser().getUid().toString()).addListenerForSingleValueEvent(new ValueEventListener() {
                                    @Override
                                    public void onDataChange(@NonNull  DataSnapshot snapshot) {
                                        ...
                                        {
                                            database.getReference().child("Teacher").orderByKey().equalTo(auth.getCurrentUser().getUid().toString()).addListenerForSingleValueEvent(new ValueEventListener() {
                                                @Override
                                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                    if(snapshot.hasChildren()) {
                                                        progressDialog.hide();
